...
Date filters allow filtering by date fields (created_at, valid_at, invalid_at, expired_at):
  ...
  Examples:
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		minFactRating, _ := cmd.Flags().GetFloat64("min-fact-rating")
		...
		if cmd.Flags().Changed("min-fact-rating") {
			req.MinFactRating = zep.Float64(minFactRating)
		}
...
func init() {
	...
	graphSearchCmd.Flags().Float64("min-fact-rating", 0, "Minimum fact rating to filter results")
